 ...
 * SEI 多价位监控警报（延迟确认）
 * 
 * 来源: alt-report-SEI-2026-05-13-2209.md (即时分析)
 * 报告观点: "空头趋势加速，$0.0682跌破确认。维持持仓等止盈。
 *           TP1 $0.06585距当前2.1%，TP2 $0.06252。超卖反弹风险存在但非反转。"
 */
...
const PRICE_LEVELS = [
  // 下方价位（做空方向，价格下跌触发）
  { price: 0.06585, type: 'support', label: 'TP1止盈位',
    action: '第一止盈触发，平仓50%', priority: 'high',
    confirmPolicy: 'touch', confirmMs: 3 * 60 * 1000 },
    
  { price: 0.0655, type: 'support', label: '4H 61.8%回撤',
    action: '跌破$0.0655，空头加速至$0.0625', priority: 'high',
    confirmPolicy: 'hold', confirmMs: 10 * 60 * 1000 },
    
  { price: 0.06252, type: 'support', label: 'TP2止盈位',
    action: '第二止盈触发，平仓剩余', priority: 'high',
    confirmPolicy: 'touch', confirmMs: 3 * 60 * 1000 },
  
  // 上方价位（反弹风险）
  { price: 0.0682, type: 'resistance', label: '4H 50%回撤',
    action: '反弹站上$0.0682，空头受阻', priority: 'high',
    confirmPolicy: 'hold', confirmMs: 15 * 60 * 1000 },
    
  { price: 0.0694, type: 'resistance', label: '前支撑转阻力',
    action: '反弹站上$0.0694，短期反弹风险上升', priority: 'medium',
    confirmPolicy: 'hold', confirmMs: 15 * 60 * 1000 },
    
  { price: 0.0735, type: 'resistance', label: '止损位附近',
    action: '接近止损$0.07356，做空逻辑失效预警', priority: 'critical',
    confirmPolicy: 'hold', confirmMs: 10 * 60 * 1000 }
];
...
// ============================================================
const STABILITY = {
  maxRetracePercent: 0.15,    // SEI波动较大，回穿容忍度0.15%
...
module.exports = {
  ...
  async check() {
    ...
    try {
      // ⭐ 使用SWAP合约数据
      const klines = await api.getOKXKlines('SEI', '1m', 3, 'SWAP');
      ...
      console.log(`[🔍警报检查] [API] OKX获取SEI-SWAP 3根1分钟K线 | [进度] ${this.name} | 区间: $${periodLow.toFixed(5)}-$${periodHigh.toFixed(5)} | 当前: $${latestPrice.toFixed(5)} | 状态: ${statusStr} | 触发: ${confirmedLevels.length > 0} | [来源] alt-report-SEI-2026-05-13-2209: "空头趋势加速，$0.0682跌破确认，维持持仓等止盈"`);
  ...
  async collect() {
    try {
      ...
      const ticker = await api.getOKXTicker('SEI', 'SWAP');
      const klines15m = await api.getOKXKlines('SEI', '15m', 8, 'SWAP');
      
      let oiData = null, takerData = null, lsData = null;
      try {
        oiData = await api.getOKXOpenInterest('SEI');
        takerData = await api.getOKXTakerRatio('SEI');
        lsData = await api.getOKXLongShortRatio('SEI');
      } catch (e) { /* 静默 */ }
  ...
  async trigger(data) {
    ...
    const jobName = `alert-SEI-price-${Date.now()}`;
    ...
    console.log(`[SEI警报触发] 已派发即时分析任务: ${jobName} | 确认触发价位: ${data.triggeredLevels.length}个`);
