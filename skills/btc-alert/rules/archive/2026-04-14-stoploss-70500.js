...
module.exports = {
  ...
  async check() {
    ...
    try {
      ...
      console.log(`[止损监控] 当前价格: $${ticker.price}, 止损位: $${TARGET_PRICE}`);
  ...
  async collect() {
    try {
      ...
      return {
        ...
        alertType: '止损触发',
        significance: '价格触及止损位$70,500，建议立即平仓止损',
        recommendation: '触发止损，评估是否立即平仓或根据市场走势调整止损位'
      };
    } catch (error) {
  ...
  async trigger(data) {
    ...
    const jobName = `alert-sl-${Date.now()}`;
