/**
 * STRK 延迟确认多价位监控警报
 * 做空持仓监控：止损位 + 关键阻力 + 止盈目标 + 深度支撑
 *
 * 来源：alt-report-STRK-2026-05-11-1218.md
 * 报告观点："脉冲行情已过顶回落，做空信号已确认。SL=$0.0540，TP1=$0.0475，TP2=$0.0415"
 */
...
const PRICE_LEVELS = [
  // 上方价位（做空持仓的风险端）
  { price: 0.0540, type: 'resistance', label: '止损位',
    action: '止损触发，全平', priority: 'critical',
    confirmPolicy: 'instant', confirmMs: 0 },

  { price: 0.0538, type: 'resistance', label: '4H 38.2%斐波那契',
    action: '回落结构失败，评估止损', priority: 'high',
    confirmPolicy: 'hold', confirmMs: 15 * 60 * 1000 },

  // 下方价位（做空持仓的盈利端）
  { price: 0.0477, type: 'support', label: '4H 61.8%斐波那契/TP1区',
    action: '接近TP1，评估止盈', priority: 'high',
    confirmPolicy: 'touch', confirmMs: 5 * 60 * 1000 },

  { price: 0.0415, type: 'support', label: 'TP2/起涨点',
    action: '接近TP2，评估全平', priority: 'high',
    confirmPolicy: 'touch', confirmMs: 5 * 60 * 1000 },

  { price: 0.0380, type: 'support', label: '日线EMA26区域',
    action: '深度回落，趋势加速', priority: 'medium',
...
module.exports = {
  ...
  async check() {
    ...
    try {
      ...
      console.log(`[🔍警报检查] [API] OKX获取STRK ${klines.length}根1分钟K线 | [进度] ${this.name} | 区间: $${periodLow.toFixed(5)}-$${periodHigh.toFixed(5)} | 当前: $${latestPrice.toFixed(5)} | 状态: ${statusStr} | 触发: ${confirmedLevels.length > 0} | [来源] 05-11 STRK分析: "脉冲行情已过顶回落，SL=$0.0540，TP1=$0.0475，TP2=$0.0415"`);
