...
`data-monitor` 进程（PM2 常驻，5s 轮询）持续监控实盘仓位变化。OCO 止盈/止损触发导致仓位减少或归零时，系统自动检测并执行相应动作：
- 仓位部分减少 → 派发即时分析（stage1-instant → stage2 → stage3）
- 仓位完全归零 → 自动归档周期 + 规则清零 + 复盘 cron

阶段四**无需**为 SL/TP 价位设立价格警报——这层监控已由 data-monitor 替代。
...
| ☐ SL/TP 是否已由 data-monitor 接管？ | 阶段四无需设立 SL/TP 价位警报 | 重复设立已被自动化的监控 |
...
>
> **每个价位必须分配确认策略，否则就是给假突破开门。入场触发位用 hold（15-20min），整数关口用 deep_hold。**
>
> **SL/TP 的仓位归零监控由 data-monitor 自动处理，阶段四无需设立 SL/TP 价位警报。**
