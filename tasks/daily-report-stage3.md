...
| 日报文件路径 | `日报文件:` | 读取分析报告 |
| 持仓文件路径 | `持仓文件:` | 读取实盘持仓 |
| 操作建议类型 | `操作建议类型:` | 预判执行类型 |
| 入场条件 | `入场条件:` | 判断是否立即执行 |

**预解析优势：**
- 如果 `入场条件: 等待触发`，可直接跳过仓位操作执行
- 如果 `操作建议类型: 观望`，可直接跳过仓位操作执行
...
[$NOW] [阶段三] 保底路径: 周期=${CYCLE_DIR}, 报告=${REPORT_FILE}
```
...
| 日报文件 | `test -f $REPORT_FILE` | ⛔ ERROR，无法继续 |
| 持仓文件 | `test -f $POSITIONS_FILE` | ⚠️ WARN，假设无持仓 |

**日志记录：**
```
[$NOW] [阶段三] 参数来源: [spawn解析 | 保底查找]
[$NOW] [阶段三] 日报文件: reports/btc-report-YYYY-MM-DD-HHMM.md
[$NOW] [阶段三] 周期ID: cycle-YYYYMMDD-XXX
```
...
**保底查找规则：**
- 活跃周期：`ls -td active/cycle-* | head -1`
- 归档周期：`ls -td archived/cycle-* | head -1`（若无活跃周期）
- 周期状态：根据路径判断（active路径 → active，archived路径 → archived）
- 持仓状态：读取 `${CYCLE_DIR}/positions.json` 中的「当前持仓数」
