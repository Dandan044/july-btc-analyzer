...
**⚠️ 优先从上一步消息解析 data-manifest 路径，若解析失败则从本地默认路径查找（保底措施）。**
...
| 仓位 | xxxu (225u~450u，以300u账户为基准，越大的把握仓位越大) |
| 止损 | $xxx (-xx%) |
| 止盈1 | $xxx (+xx%，平仓xx%) |
| 止盈2 | $xxx (+xx%，平仓剩余) |
...
1. **⭐ 优先解析上一步消息获取清单路径**：从阶段一结束后传递的消息中获取 data-manifest 路径
2. **保底措施必须执行**：上一步消息解析失败时，从本地默认路径查找清单
