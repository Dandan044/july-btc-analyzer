...
echo "[$NOW] [阶段二] [激进版] 开始执行 - 交叉验证分析" >> logs/alt-${COIN}-process.log
```
...
本版本不读取 `TRADE_LESSONS.md`。历史教训已内化为常规风控逻辑（盈亏比 ≥ 1.2、波动率 ≤ 35% 红线），无需额外审查。
...
> **激进版说明：** REJECT 阈值已从 25% 放宽至 35%，仅极高山寨币（4H ATR > 17.5% × X=2.0）才会触发。
